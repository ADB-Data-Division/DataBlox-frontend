/**
 * Pure cache-key helpers for the coastal vessels choropleth (tab 2).
 *
 * Incident 2026-09-25: scrubbing the weekly time slider with country=PHL
 * fired a full `/spatial/series` prefetch per tick plus per-tick
 * `/spatial/slice` fallbacks, restarting the API container. The
 * series/slice effects in `page-content.tsx` now rely on these helpers so
 * that scrubbing serves slices from the batch cache while country / grain /
 * tab / AOI changes still trigger exactly one fetch.
 *
 * Everything here is pure (no React, no fetch) so it is unit-testable in
 * isolation. See `src/tests/coastal/spatial-cache-vessels.test.ts`.
 */
 ...
 * Shape: `country_day_indicator_grain_aoi` (vessels scope uses
 * `country_day_vessels_grain_aoi`).
 */
